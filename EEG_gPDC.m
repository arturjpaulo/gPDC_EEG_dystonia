...
raw=eeg.io.loadDirectory ('C:\Users\artur\Documents\gPDC_escrita\');
%%modificar linha de que faz resampling
%% identificar resting e resampling
raw1=identify_escrita_eeg(raw);
clear raw
raw1=raw1';
%%
%
...
%Filtering
for i=1:length(raw2)
 raw3(i).data = eegfilt (raw2(i).data',raw2(i).Fs, 0.5, 50);


save_graph ={}; 
mkdir('C:\Users\artur\Documents\PDC')
addpath(genpath('C:\Users\artur\Documents\PDC'))
...
begin= (raw3(p).stimulus.values{1,2}.onset(1))*raw3(p).Fs;

final= begin+(raw3(p).Fs *60);


% select channels
c= raw3(p).data(:,begin:final);
raw2(p).description
FC5=c(6,:);
CP5=c(11,:);
FC6=c(28,:);
...
COI=vertcat(FC5,CP5,FC6,CP6,Fz,Cz);

COI= COI ( ~any(isnan(COI)| isinf (COI),2),:);
u=COI';
chLabels={'FC5','CP5','FC6','CP6','Fz','Cz'};
...
[Tr_gct, pValue_gct, Tr_igct, pValue_igct] = gct_alg(u,A,pf,gct_signif, ...
                                                         flgPrintResults);
% 
%   writematrix(Tr_gct,['C:\Users\artur\Documents\gPDC_escrita\gPDC_distonia_partial_results\Control_tables\' raw3(p).description(end-18:end-5) 'Tr_gct.csv'])  
%   writematrix(pValue_gct,['C:\Users\artur\Documents\gPDC_escrita\gPDC_distonia_partial_results\Control_tables\' raw3(p).description(end-18:end-5) 'pValue_gct.csv']) 
%==========================================================================
...
strWindowName = ['pdc Analysis subj:' raw3(p).description(end-18:end-5)];
...
%       'Name', strWindowName )
% 
%    [hxlabel hylabel] = pdc_xplot(c,flgPrinting,fs,w_max,chLabels,kflgColor);
%    
...
%    
% end;
%   exportgraphics(h,['C:\Users\artur\Documents\gPDC_escrita\gPDC_distonia_partial_results\Control_graphs\' raw2(p).description(end-18:end-5) '.png'])
